...
export default function InventoryPage() {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/amenities/inventory`, {
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint = editingId
      ? `${API_BASE}/api/amenities/inventory/${editingId}`
      : `${API_BASE}/api/amenities/inventory`;
 ...
 // ✅ Add this here to verify the ID

  try {
    const res = await fetch(`${API_BASE}/api/amenities/inventory/${id}`, {
      method: 'DELETE',
    });
  ...
  setInventory((prev) => prev.filter((item) => item.id !== id)); // Still remove it from UI
