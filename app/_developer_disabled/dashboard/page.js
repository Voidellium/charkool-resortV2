...
function ModelsTab() {
  ...
  const [activeSection, setActiveSection] = useState('resort'); // 'resort' or 'interiors'

  const fetchModels = async () => {
    try {
      setError(null);
      
      // Fetch both model list and config
      const [modelsRes, configRes] = await Promise.all([
        fetch('/api/developer/models'),
        fetch('/api/developer/models/config')
      ]);
      ...
      if (Array.isArray(modelsData)) {
        ...
        console.error('API returned non-array data:', modelsData);
        setModels([]);
    ...
    <div className="models-tab">
      ...
      <div className="section-toggle">
        <button
          ...
          className={`toggle-btn ${activeSection === 'resort' ? 'active' : ''}`}
        >
          🏨 Resort Main Map
        </button>
        <button
          onClick={() => setActiveSection('interiors')}
          className={`toggle-btn ${activeSection === 'interiors' ? 'active' : ''}`}
        >
          🏠 Room Interiors
        </button>
      </div>
        ...
        <>
          <div className="upload-section-inline">
            ...
            <label htmlFor="model-upload" className={`upload-btn ${uploading ? 'disabled' : ''}`}>
              {uploading ? 'Uploading...' : '📁 Upload Resort Map Model'}
            </label>
          </div>
          ...
          {(!models || models.length === 0) && !error && (
            <div className="empty-state">
              ...
              <div className="empty-title">No resort map models found</div>
              <div className="empty-subtitle">Upload your resort map GLTF model to get started</div>
            </div>
          )}
        </>
      )}
      ...
      {/* Interiors Section */}
      {activeSection === 'interiors' && <InteriorsSection modelConfig={modelConfig} fetchModels={fetchModels} />}
...
// Interiors Section Component
function InteriorsSection({ modelConfig, fetchModels }) {
  const [interiorPaths, setInteriorPaths] = useState({
  ...
  useEffect(() => {
    if (modelConfig && Array.isArray(modelConfig)) {
      const config = {
        Teepee: modelConfig.find(c => c.modelType === 'INTERIOR_TEEPEE')?.modelPath || '/models/Interior_Teepee.glb',
        Villa: modelConfig.find(c => c.modelType === 'INTERIOR_VILLA')?.modelPath || '/models/Interior_Villa.glb',
        Loft: modelConfig.find(c => c.modelType === 'INTERIOR_LOFT')?.modelPath || '/models/Interior_Loft.glb'
      };
      setInteriorPaths(config);
    }
  }, [modelConfig]);

  const handlePathChange = async (roomType, newPath) => {
    setSaving(true);
    try {
      const modelTypeMap = {
        'Teepee': 'INTERIOR_TEEPEE',
        'Villa': 'INTERIOR_VILLA',
        'Loft': 'INTERIOR_LOFT'
      };

      const response = await fetch('/api/developer/models/config', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          modelType: modelTypeMap[roomType],
          modelPath: newPath
        })
      });

      if (response.ok) {
        alert(`${roomType} interior model updated successfully!`);
        setInteriorPaths(prev => ({ ...prev, [roomType]: newPath }));
        setEditingRoom(null);
        fetchModels(); // Refresh the config
      } else {
        alert('Failed to update model path');
      }
    } catch (error) {
      alert(`Error: ${error.message}`);
    } finally {
  ...
  const rooms = [
    { name: 'Teepee', icon: '🏕️', description: 'Teepee room interior model' },
    { name: 'Villa', icon: '🏰', description: 'Villa room interior model' },
    { name: 'Loft', icon: '🏠', description: 'Loft room interior model' }
  ];
  ...
  return (
    <div>
      <div style={{ marginBottom: '24px' }}>
        <div style={{ 
          padding: '16px', 
          background: 'linear-gradient(135deg, rgba(254, 190, 84, 0.1) 0%, rgba(235, 213, 145, 0.1) 100%)',
          borderRadius: '12px',
          border: '2px solid rgba(254, 190, 84, 0.3)'
        }}>
          <div style={{ fontSize: '14px', color: '#0b3a4a', marginBottom: '8px', fontWeight: '600' }}>
            ℹ️ About Interior Models
          </div>
          <div style={{ fontSize: '13px', color: '#4b5563', lineHeight: '1.6' }}>
            These models are displayed when guests view room interiors in the 3D viewer and virtual tour.
            Enter the path to your model file (e.g., /models/Interior_Teepee.glb) to switch between different versions.
          </div>
        </div>
      </div>
      ...
      <div className="interiors-grid">
        {rooms.map((room) => (
          <div key={room.name} className="interior-card">
            ...
            <div className="path-section">
              <label className="path-label">Model Path:</label>
              {editingRoom === room.name ? (
                <div className="edit-controls">
                  <input
                    type="text"
                    value={interiorPaths[room.name]}
                    onChange={(e) => setInteriorPaths(prev => ({ ...prev, [room.name]: e.target.value }))}
                    className="path-input"
                    placeholder="/models/Interior_Room.glb"
                  />
                  <div className="edit-buttons">
                    <button
                      onClick={() => handlePathChange(room.name, interiorPaths[room.name])}
                      disabled={saving}
                      className="save-btn"
                    >
                      {saving ? '💾 Saving...' : '💾 Save'}
                    ...
                    <button
                      onClick={() => {
                        ...
                        const original = modelConfig?.find(c => c.modelType === `INTERIOR_${room.name.toUpperCase()}`)?.modelPath || `/models/Interior_${room.name}.glb`;
                        setInteriorPaths(prev => ({ ...prev, [room.name]: original }));
                      }}
                ...
                <div className="view-controls">
                  <code className="path-display">{interiorPaths[room.name]}</code>
                  <button
                    onClick={() => setEditingRoom(room.name)}
                    className="edit-btn"
                  >
                    ✏️ Edit
                  </button>
      ...
      <style jsx>{`
        ...
        .path-input:focus {
          outline: none;
          border-color: #FEBE54;
          box-shadow: 0 0 0 3px rgba(254, 190, 84, 0.1);
        }
        .path-display {
          display: block;
          padding: 10px 12px;
