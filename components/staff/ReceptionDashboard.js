...
import { useFrontDeskNav } from '@/components/staff/FrontDeskNavContext';
...
export default function ReceptionDashboard() {
  ...
  const navigationGuard = useNavigationGuard({
    ...
    context: 'logout',
    message: 'Are you sure you want to log out of your front desk dashboard?'
  });
  ...
  const { setNavExtras } = useFrontDeskNav();
  ...
  const getFilteredBookings = () => {
    ...
    if (searchTerm) {
      ...
      filteredBookings = filteredBookings.filter(booking => 
        ...
        (Array.isArray(booking.remarks) ? booking.remarks.some(r => r.content?.toLowerCase().includes(normalizedSearch)) : booking.remarks?.toString().toLowerCase().includes(normalizedSearch))
      );
    }
  ...
  const openRemarksModal = (booking) => {
    setBookingRemarks(Array.isArray(booking.remarks) ? booking.remarks.map(r => r.content).join('\n') : String(booking.remarks || ''));
  ...
  const operationalBadgeCount =
    notifications.pendingCheckIns.length +
    notifications.pendingCheckOuts.length +
  ...
  useEffect(() => {
    setNavExtras({
      ...
      onShiftSummary: generateShiftSummary,
      onOperationalNotificationsClick: () => setShowNotificationPanel((v) => !v),
    });
  }, [operationalBadgeCount, setNavExtras]);
  ...
  useEffect(() => {
    async function calculateCreateTotal() {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          setCreateTotalPrice(data.totalCostWithAddons || data.totalAfterDiscount || data.totalPrice || 0);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    async function calculateTotal() {
      try {
        ...
        if(res.ok) {
          const data = await res.json();
          setTotalPrice(data.totalCostWithAddons || data.totalAfterDiscount || data.totalPrice || 0);
        }
      } catch (error) {
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '10px' }}>
              ...
              <div style={{ marginTop: '8px' }}>
                {modalData && modalData.remarks && Array.isArray(modalData.remarks) && modalData.remarks.length > 0 ? (
                  modalData.remarks.map((r) => (
                    <div key={`remark-${r.id}`} style={{ marginBottom: '8px', padding: '8px', background: '#f3f4f6', borderRadius: '6px' }}>
                      <div style={{ fontSize: '0.9rem', color: '#374151' }}>
                        <strong>{(r.author && r.author.name) ? r.author.name : r.authorRole}</strong>
                        <span style={{ color: '#6b7280', marginLeft: '8px' }}>{new Date(r.createdAt).toLocaleString()}</span>
                      </div>
                      <div style={{ marginTop: '6px' }}>{JSON.stringify(r.content)}</div>
                    </div>
                  ))
                ) : (
              ...
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  const payload = {
                    ...
                    cottage,
                    nights,
                    // Walk-in created by receptionist should be auto-confirmed
                    status: 'Confirmed',
                    ...
                    <div style={{ display: 'grid', gap: '20px' }}>
                      ...
                      <div>
                        ...
                        <OptionalAmenitiesSelector
                          amenitiesData={optionalAmenitiesData}
                      ...
                      <div>
                        ...
                        <RentalAmenitiesSelector
                          amenitiesData={rentalAmenitiesData}
              ...
              getPaginatedBookings().map((booking) => (
                <div key={booking.id} className={`booking-card ${booking.status?.toLowerCase() || 'pending'}`}>
                  ...
                  <div className="booking-details">
                    ...
                    {booking.remarks && (Array.isArray(booking.remarks) ? booking.remarks.length > 0 : !!booking.remarks) && (
                      <div className="detail-row remarks">
                        <svg viewBox="0 0 20 20" fill="currentColor">
                          <path fillRule="evenodd" d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.338-3.123C2.493 12.767 2 11.434 2 10c0-3.866 3.582-7 8-7s8 3.134 8 7zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clipRule="evenodd" />
                        </svg>
                        <span className="detail-value remarks-text">
                          {Array.isArray(booking.remarks)
                            ? booking.remarks.map(r => r.content).join(', ')
                            : String(booking.remarks)}
                        </span>
                      </div>
                    )}
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '500px' }}>
            ...
            <div className="guest-quick-view">
              ...
              {quickViewGuest.remarks && (Array.isArray(quickViewGuest.remarks) ? quickViewGuest.remarks.length > 0 : !!quickViewGuest.remarks) && <p><strong>Remarks:</strong> {Array.isArray(quickViewGuest.remarks) ? quickViewGuest.remarks.map(r => r.content).join(', ') : String(quickViewGuest.remarks)}</p>}
                  ...
                  return (
                    <>
                      ...
                      {modalData.rentalAmenities && Array.isArray(modalData.rentalAmenities) && modalData.rentalAmenities.map((ra, idx) => (
                        <li key={`rental-${idx}`}>
                          {ra.rentalAmenity.name} x{ra.quantity} {ra.hoursUsed ? `(${ra.hoursUsed}h)` : ''}: ₱{(calculateRentalAmenityTotalCents(ra) / 100).toFixed(0)}
                        </li>
                      ))}
                      {discountAmount > 0 && (
